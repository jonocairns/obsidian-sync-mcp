...
- **Bounded attachment reads** — attachment reads sit behind the same access boundary as note reads: a bearer token when `MCP_AUTH_TOKEN` is set, otherwise the Host/Origin checks alone, which any non-browser client that can reach the port satisfies. Clients inside that boundary can read existing PNG, JPEG, WebP and PDF files through `read_attachment` or versioned MCP resources. The server checks the content signature and container integrity and caps raw bytes, which is what bounds a read; it does not measure images, and never writes decrypted temporary files. Attachment paths and bytes are not logged by the application. Embed-target resolution enumerates attachment metadata and may be expensive for a very large vault; exact-path reads avoid that enumeration.
