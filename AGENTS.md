...
- **`lib/livesync-commonlib` is a git submodule reached via the `@lib/*` alias.** A "cannot find module `@lib/...`" error means the submodule isn't initialized (`git submodule update --init --recursive`), not a broken import.
